...
## Determine compiler flags for different version.
if ($ENV{'LATEX_MODE'} eq 'ONLINE') {
    print "--- Building: ONLINE version ---\n";
    set_tex_cmds("--shell-escape --file-line-error -interaction=nonstopmode %O \"\\def\\ONLINEVERSION{1} \\input{%S}\"");
} else {
    print "--- Building: PRINT version ---\n";
